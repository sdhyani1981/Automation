...
public class StepDefinitionBLackList {
	...
	static {
		try {
			...
			File file = new File("//Users//shivanidhyani//eclipse-workspace//Automation//src//main//java//config//blacklist.properties");
	...
	public void i_am_a_fraud_analyst_with_auth_token(String token) {
		...
		Assert.assertNotNull("Can't add to blacklist, invalid token - " + token, ids.get(0));
	}
	...
	public void check_it_against_the_company_blacklist() {
		...
		RequestSpecification request = RestAssured.given()
				.contentType("application/json")
				.queryParam(scenarioContext.getUserinfotype()
						, scenarioContext.getUserinfo());
	...
	public void item_created_successfully() {
		Assert.assertNotNull("Null id returned even after response had Status code 201", scenarioContext.getBlackListItemId());
	}	
	
	@Then("Permission is denied")
	public void permission_is_denied() {
		Assert.assertEquals("PERMISSION_DENIED", scenarioContext.getCode());
	}	
